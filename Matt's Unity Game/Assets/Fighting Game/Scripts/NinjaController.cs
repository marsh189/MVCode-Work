...
public class NinjaController : MonoBehaviour
{
    ...
    [SerializeField] private Collider2D m_SlideEnableCollider;                 // A collider that will be enabled when crouching
    ...
    public void Move(float move, bool crouch, bool jump)
    {
        ...
        if (!GetComponent<NinjaMovement>().animator.GetCurrentAnimatorStateInfo(0).IsName("player_attack_Sword"))
        {
            // If crouching
            if (crouch && Mathf.Abs(move) < 0.01)
            {
                if (!m_wasCrouching)
                {
                    m_wasCrouching = true;
            ...
            else if (crouch && Mathf.Abs(move) > 0.01)
            {
                if (!m_wasCrouching)
                {
                    m_wasCrouching = true;
